...
function SummaryTable(props: {
	...
	return (
		<section className="rounded-[24px] border border-slate-200/70 bg-white/90 p-5 shadow-[0_16px_50px_-32px_rgba(15,23,42,0.55)] dark:border-slate-800 dark:bg-slate-950/70">
			<h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{props.title}</h2>
...
export function SummaryView(props: { bundle: ReadyBundle }) {
		...
		<div className="space-y-6">
			<section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
				...
				<MetricCard
					...
					note={
						summary.hasLlm
							? "The LLM companion report is available, so hotspot tables use companion-report reachability."
							: "No LLM companion report was found, so hotspot tables use graph-derived reachable-module counts."
					}
				/>
			</section>
			...
			<section className="grid gap-4 xl:grid-cols-2">
				<SummaryTable
					...
					description={
						summary.hasLlm
							? "Approximate hotspots ranked by reachable modules from the companion report."
							: "Graph-derived ranking based on how many unique modules each file can reach through its import chain."
					}
					rows={summary.hotspots}
					valueLabel="Reachable"
					getLink={(row) => ({ to: "/trace", search: { node: row.path, package: "", limit: 10 } })}
				/>
				<SummaryTable
			...
			<section className="grid gap-4 xl:grid-cols-2">
				...
				<SummaryTable
					...
					valueLabel="Nodes"
					getLink={(row) => ({ to: "/trace", search: { package: row.label, node: "", limit: 10 } })}
				/>
			</section>
			...
			<SummaryTable
				...
				getLink={(row) => ({
					to: "/graph",
					search: { cluster: row.label, externalGrouping: "package" },
				})}
			/>
		</div>
	);
